/**
 * Modern Format Adapter
 *
 * Implements format rules for Modern constructed:
 * - 60-card minimum mainboard
 * - 15-card sideboard maximum
 * - 4-copy limit (except basic lands)
 * - Modern legality based on sets from 8th Edition forward
 */
...
export class ModernAdapter implements FormatAdapter {
  readonly format: FormatType = 'modern';
  ...
  getLegalityStatus(card: Card): LegalityStatus {
    if (!ModernAdapter.isLegalitiesRecord(card.gameData)) {
      return 'not_legal';
    }

    const status = card.gameData.legalities.modern;
  ...
  validateDeck(deck: DeckWithCards): ValidationResult {
    ...
    for (const deckCard of deck.cards) {
      if (!this.isLegal(deckCard.card)) {
        errors.push({
          code: 'CARD_NOT_LEGAL',
          message: `${deckCard.card.name} is not legal in Modern`,
  ...
  getColorConstraint(_deck: DeckWithCards): ColorConstraint {
    // Modern doesn't enforce color identity like Commander
  ...
  isColorCompatible(card: Card, constraint: ColorConstraint): boolean {
    // Modern doesn't enforce color identity
  ...
  getOptimalCMCPosition(deck: DeckWithCards): number {
    ...
    if (mainboardCards.length === 0) {
      // Early deck: prefer 1-2 CMC cards for Modern's faster pace
      return 2;
    }
    ...
    const avgCMC = totalCards > 0 ? totalCMC / totalCards : 2;

    // Modern is typically faster than Standard, target 2.5-3.0 CMC
    const targetAvgCMC = 2.75;
